...
bool Map::is_solid(glm::vec3 position, float* penetration_x, float* penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    // wood floors
    if (tile == 80) return false;

    if (tile == 141 || tile == 142) return false;

    //bedroom rug
    if (tile == 230 || tile == 229 || tile == 246 || tile == 245) return false;

    // living room rug
    if (tile == 215 || tile == 216 || tile == 217 || tile == 231 || tile == 232 || tile == 233 || tile == 247 || tile == 248 || tile == 249) return false;

    // coffee table legs
    if (tile == 77 || tile == 79) return false;
